...
using CalamityOverhaulLegacy.Content.DamageModify;
using CalamityOverhaulLegacy.Content.PRTTypes;
using InnoVault.PRT;
...
namespace CalamityOverhaulLegacy.Content.Items.Ranged.HeavenfallLongbows
{
    internal class InfiniteArrow : ModProjectile, IPrimitiveDrawable
    {
            ...
            if (!VaultUtils.isServer) {
                for (int i = 0; i < 16; i++) {
                    ...
                    PRTLoader.NewParticle<PRT_Light>(pos, particleSpeed
                        , VaultUtils.MultiStepColorLerp(Main.rand.NextFloat(1), HeavenfallLongbow.rainbowColors), Main.rand.NextFloat(0.3f, 0.7f)).Configure(30, 1, 1.5f, hueShift: 0.0f);
                }
            }
        ...
        void IPrimitiveDrawable.DrawPrimitives() {
            ...
            effect.Parameters["uBaseImage"].SetValue(CWRAsset.StarTexture.Value);
            effect.Parameters["uFlow"].SetValue(CWRAsset.Airflow.Value);
            effect.Parameters["uGradient"].SetValue(CWRAsset.DarklightGreatsword_Bar.Value);
